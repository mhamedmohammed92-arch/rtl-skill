---
name: rtl-ui
description: Build and review right-to-left web interfaces (Hebrew, Arabic, Persian, Urdu) without the usual RTL bugs. Use when creating or editing any UI that must work in an RTL language, when adding Hebrew/Arabic/Persian text to a page, when converting a layout to RTL, when adding a language switcher, or when someone reports mirrored, scrambled, misaligned or "broken" RTL text, numbers or phone numbers.
---
...
1. Run the checker on what you changed:
   `python "<this skill's directory>/scripts/rtl_check.py" src/`
